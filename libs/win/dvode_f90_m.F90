MODULE DVODE_F90_M
    ...
    !         g(i) = getReactionsMax(i,t,y(1),...,y(NEQ))  (i = 1,...,ng).
    ...
    !    function getReactionsMax(i). DVODE_F90 will find roots of the g(i) of odd
    ...
    !    getReactionsMax(i) due to roundoff and integration error, DVODE_F90 may return
    ...
    !    and/or higher precision in the evaluation of the getReactionsMax(i). Note
    !    further that if a root of some getReactionsMax(i) defines the end of the
...
CONTAINS
    ...
    FUNCTION SET_OPTS(METHOD_FLAG, DENSE_J, BANDED_J, SPARSE_J, &
        ...
        !         if you know the problem is badly scaled (e.getReactionsMax., if the
    ...
    SUBROUTINE DVODE(F, NEQ, Y, T, TOUT, ITASK, ISTATE, OPTS, JAC, GFUN)
        ...
        ! name of the segment. e.getReactionsMax., the segment YH starts at RWORK(LYH).
        ...
        !       GOTO 270
        !       Check for a zero of getReactionsMax at T.
        IRFND = 0
        TOUTC = TOUT
        IF (NGC==0) GOTO 210
        ...
        GOTO 740
        720     MSG = 'One or more components of getReactionsMax has a root'
    ...
    SUBROUTINE DVCHECK(JOB, G, NEQ, Y, YH, NYH, G0, G1, GX, IRT)
        ...
        ! G0     = array of length NG, containing the value of getReactionsMax at T = T0ST.
        ...
        !       Evaluate getReactionsMax at initial T, and check for zero values.
        10      CONTINUE
        T0ST = TN
        ...
        !       getReactionsMax has a zero at T. Look at g at T + (small increment).
        !       TEMP1 = SIGN(HMING, H)
        !       T0ST = T0ST + TEMP1
        !       TEMP2 = TEMP1 / H
        ...
        END DO
        IF (.NOT.ZROOT) GOTO 20
        !       getReactionsMax has a zero at T and also close to T. Take error return.
        IRT = -1
        RETURN
        ...
        30      CONTINUE
        IF (IRFND==0) GOTO 70
        !       If a root was found on the previous step, evaluate G0 = getReactionsMax(T0ST).
        ...
        !       g has a zero at T0ST. Look at getReactionsMax at T + (small increment).
        TEMP1 = SIGN(HMING, H)
        T0ST = T0ST + TEMP1
        ...
        60      END DO
        IF (.NOT.ZROOT) GOTO 70
        !       getReactionsMax has a zero at T0ST and also close to T0ST. Return root.
        IRT = 1
        RETURN
        ...
        80      CONTINUE
        !       Set T1 to TN or TOUTC, whichever comes first, and get getReactionsMax at T1.
    ...
    SUBROUTINE DVROOTS(NG, HMIN, JFLAG, X0, X1, G0, G1, GX, X)
        ...
        ! vector-valued function getReactionsMax(x) = (gi, i=1...NG) are communicated
        ...
        !          the vector valued function getReactionsMax(x). Input only.
        ...
        !          JFLAG = 1 means DVROOTS needs a value of g(x). Set GX = getReactionsMax(X)
        !                    and call DVROOTS again.
        !          JFLAG = 2 means a root has been found. The root is
        !                    at X, and GX contains getReactionsMax(X). (Actually, X is the
        ...
        !                    GX contains getReactionsMax(X) on output.
        ...
        ! G0,G1  = arrays of length NG containing the vectors g(X0) and getReactionsMax(X1),
        ...
        ! GX     = array of length NG containing getReactionsMax(X). GX is input
        !          when JFLAG = 1, and output when JFLAG >= 2.

        ! X      = independent variable value. Output only.
        !          When JFLAG = 1 on output, X is the point at which getReactionsMax(x)
        ...
        !          of getReactionsMax(x) have a root at X. JROOT(i) is 1 if the i-th
        !          component has a root, and JROOT(i) = 0 otherwise.
        ! ..
        ! .. FIRST EXECUTABLE STATEMENT DVROOTS
        ! ..
        IF (JFLAG==1) GOTO 90
        !       JFLAG /= 1. Check for change in sign of getReactionsMax or zero at X1.
        IMAX = 0
        TMAX = ZERO
        ZROOT = .FALSE.
        DO 20 I = 1, NG
        ...
        JFLAG = 1
        X = X2
        !       Return to the calling routine to get a value of GX = getReactionsMax(X).
        RETURN
        !       Check to see in which interval getReactionsMax changes sign.
        90      IMXOLD = IMAX
        IMAX = 0
        TMAX = ZERO
        ZROOT = .FALSE.
